    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="bg-white p-4 rounded-lg shadow-md flex items-center">
        ...
        <div class="ml-4">
          ...
          <p class="text-xl font-bold text-gray-800">
            {{ formatCurrency(totalInvertidoActivo) }}
          </p>
        </div>
      </div>
    </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-if="inversionesFiltradas.length === 0">
              <td colspan="5" class="text-center py-10 text-gray-500">
                No hay inversiones que coincidan con el filtro.
              </td>
            </tr>
            <tr
              v-for="(inversion, idx) in inversionesFiltradas"
              :key="inversion.id"
            >
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="text-sm text-gray-500">
                  {{ inversion.beneficiary || inversion.beneficiario }}
                </div>
              </td>
              ...
              >
                {{
                  formatCurrency(
                    parseFloat(
                      inversion.invested_amount || inversion.monto || 0
                    )
                  )
                }}
              </td>
...
const { deleteInversion, addInversion, updateInversion, fetchInversiones } =
  store;

// State reactivo del store
const { inversiones, totalGanancias, totalInvertidoActivo } =
  storeToRefs(store);
...
const handleSaveInversion = async (inversionData) => {
  ...
  if (success) {
    toast.add({
      severity: "success",
      summary: "Éxito",
      detail: `Inversión ${action} correctamente.`,
      life: 3000,
    });
  } else {
    toast.add({
      severity: "error",
      summary: "Error",
      detail: `No se pudo guardar la inversión.`,
      life: 3000,
    });
  }
