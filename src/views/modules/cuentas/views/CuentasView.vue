...
const openEditModal = (cuenta) => {
  ...
  const tipoCuenta = tiposCuenta.value.find(
    (t) => t.nombre.toLowerCase() === cuenta.tipo.toLowerCase()
  );
  cuentaToEdit.value = {
    accountId: cuenta.id,
...
const handleSaveCuenta = async (cuentaData) => {
  ...
  if (success) {
    toast.add({
      ...
      summary: "Éxito",
      detail: `Cuenta ${
        isEditMode.value ? "actualizada" : "creada"
      } correctamente.`,
      life: 3000,
    });
  } else {
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(cuenta, idx) in cuentasDeBalance" :key="cuenta.id">
              ...
              <td class="px-6 py-4 text-sm">
                <Tag
                  :value="getTipoLabel(cuenta.tipo)"
                  :severity="getTipoTagSeverity(cuenta.tipo)"
                />
              </td>
              <td class="px-6 py-4 text-sm font-semibold text-gray-700">
                {{ formatCurrency(cuenta.saldo, cuenta.currencyCode) }}
              </td>
            ...
            <tr v-if="cuentasDeBalance.length === 0">
              <td colspan="4" class="px-6 py-4 text-center text-gray-500">
                No hay cuentas de activo o pasivo registradas.
              </td>
            </tr>
          </tbody>
        </table>
