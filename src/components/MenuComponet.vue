  ...
  <div>
    ...
    >
      ...
      <div class="flex items-center justify-between p-4 h-16 border-b border-gray-700 flex-shrink-0">
        ...
        <button @click="emit('toggle-collapse')" class="hidden md:block p-1 rounded-full text-gray-400 hover:bg-gray-700 hover:text-white">
      ...
      <nav class="flex-grow overflow-y-auto px-2 py-4 space-y-4">
        <div v-for="section in menuItems" :key="section.category">
          <h2 v-if="!collapsed" class="px-4 mb-2 text-xs font-semibold text-gray-500 uppercase">{{ section.category }}</h2>
          ...
          >
            ...
            <div v-if="collapsed" class="absolute left-full ml-4 px-2 py-1 bg-gray-800 text-white text-xs rounded-md shadow-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">
              {{ item.name }}
            </div>
          </router-link>
        </div>
      ...
      <div class="p-2 border-t border-gray-700 flex-shrink-0">
        <div class="flex items-center p-2 rounded-lg hover:bg-gray-800 cursor-pointer">
          <img class="h-10 w-10 rounded-full object-cover" src="https://i.pravatar.cc/100?u=BalmoreRamirez" alt="Avatar">
